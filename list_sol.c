 ...
 */
struct nodeStruct* List_createNode(void *ptr_block, int size)
{
	struct nodeStruct *pNode = malloc(sizeof(struct nodeStruct));
	if (pNode != NULL) {
		pNode->ptr_block = ptr_block;
		pNode->size = size;
		//pNode->next = NULL; //?
	}
	return pNode;
 ...
 */
struct nodeStruct* List_findNode(struct nodeStruct *head, void *ptr_block)
{
	struct nodeStruct *current = head;
	while (current != NULL) {
		if (current->ptr_block == ptr_block) {
			return current;
		}
		current = current->next;
	}
...
static _Bool doSinglePassOnSort(struct nodeStruct **headRef)
{
	...
	while (*headRef != NULL) {
		...
		// Swap needed?
		if (nodeA->ptr_block > nodeB->ptr_block){
